...
| Blockchain   | Confirmations khuyến nghị | Thời gian chờ |
| ------------ | ------------------------- | ------------- |
| **Ethereum** | 12-64 blocks              | ~3-13 phút    |
| **BSC**      | 64-128 blocks             | ~3-6 phút     |
| **Polygon**  | 128-256 blocks            | ~4-9 phút     |
| **Bitcoin**  | 6 blocks                  | ~60 phút      |
...
#### 📱 Kết nối từ Mobile (WalletConnect)

Nếu user truy cập từ mobile mà không có MetaMask, bạn có thể dùng **WalletConnect** để kết nối với các ví mobile khác.

> 📖 **Tài liệu tham khảo**: [WalletConnect Documentation](https://docs.walletconnect.com/)

```javascript
// Cài đặt: npm install @web3modal/ethers ethers

import { createWeb3Modal, defaultConfig } from "@web3modal/ethers";

// 1. Cấu hình
const projectId = "YOUR_PROJECT_ID"; // Lấy từ https://cloud.walletconnect.com
...
class NotificationService {
  constructor(provider, contractAddress, userAddress) {
    this.provider = provider;
  ...
  async start() {
    ...
    // Lắng nghe incoming transfers
    const incomingFilter = this.contract.filters.Transfer(
      null,
      this.userAddress
    );

    const incomingListener = (from, to, value, event) => {
      this.notify({
        type: "RECEIVED",
        from: from,
        amount: ethers.formatUnits(value, decimals),
    ...
    this.contract.on(incomingFilter, incomingListener);
    this.listeners.push({ filter: incomingFilter, listener: incomingListener });

    // Lắng nghe outgoing transfers
    const outgoingFilter = this.contract.filters.Transfer(
      this.userAddress,
      null
    );

    const outgoingListener = (from, to, value, event) => {
      this.notify({
        type: "SENT",
        to: to,
        amount: ethers.formatUnits(value, decimals),
    ...
    this.contract.on(outgoingFilter, outgoingListener);
    this.listeners.push({ filter: outgoingFilter, listener: outgoingListener });

    console.log("✓ Notification service started");
  }

  notify(data) {
    const { type, amount, symbol, from, to, txHash } = data;

    let message;
    if (type === "RECEIVED") {
      message = `💰 Received ${amount} ${symbol} from ${from.substring(
        0,
        10
      )}...`;
    } else {
      message = `📤 Sent ${amount} ${symbol} to ${to.substring(0, 10)}...`;
    }
    ...
    if (Notification.permission === "granted") {
      new Notification("Token Transfer", {
        body: message,
        icon: "/token-icon.png",
      });
    }

    // Custom callback (có thể là update UI, gửi webhook, etc.)
  ...
  stop() {
    if (this.contract) {
      this.listeners.forEach(({ filter, listener }) => {
  ...
  // Callback để xử lý notification
...
notificationService.onNotification((data) => {
  ...
  // Update UI, gửi webhook, lưu database, etc.
});
...
// ⚠️ Thay YOUR_CONTRACT_ADDRESS bằng địa chỉ contract thực tế
...
const SLOTS_PER_EPOCH = 32; // slots
const EPOCHS_TO_FINALIZE = 2; // epochs

const timeToFinalize = SLOT_TIME * SLOTS_PER_EPOCH * EPOCHS_TO_FINALIZE;
// = 12 × 32 × 2 = 768 seconds = 12.8 minutes

console.log("Finalize time:", timeToFinalize / 60, "minutes");
```

**Tại sao không làm nhanh hơn?**

| Nếu chỉ 1 checkpoint (6 phút) | Vấn đề                                     |
| ----------------------------- | ------------------------------------------ |
| Nhanh hơn ✅                  | Không đủ thời gian phát hiện chain fork ❌ |
|                               | Validator không kịp vote ❌                |
|                               | Dễ bị tấn công 51% ❌                      |

| Nếu 3+ checkpoints (19+ phút) | Vấn đề                                |
| ----------------------------- | ------------------------------------- |
| An toàn hơn ✅                | Quá chậm cho user experience ❌       |
|                               | Exchanges phải chờ lâu mới confirm ❌ |

→ **2 checkpoints (12 phút) là điểm cân bằng tối ưu** giữa tốc độ và bảo mật!

**Kết luận:**

```
┌──────────────────────────────────────────────────────────┐
│  12 minutes = Minimum time to ensure:                   │
│                                                          │
│  ✅ No chain fork                                       │
│  ✅ Majority of validators voted                        │
│  ✅ Network fully reached consensus                     │
│  ✅ Cannot be attacked with rollback                    │
│  ✅ Your transaction is 100% SAFE                       │
└──────────────────────────────────────────────────────────┘
```
...
**6. So sánh PoW vs PoS:**

| Aspect               | Proof of Work (PoW)    | Proof of Stake (PoS)     |
| -------------------- | ---------------------- | ------------------------ |
| **Cạnh tranh bằng**  | Sức mạnh tính toán     | Số lượng coin stake      |
| **Năng lượng**       | Rất cao (~100 TWh/năm) | Rất thấp (~0.01 TWh/năm) |
| **Hardware**         | ASIC miners ($1000+)   | Máy tính thường          |
| **Chi phí tham gia** | $10,000+               | 32 ETH (~$64,000)        |
| **51% Attack**       | Cần 51% hashrate       | Cần 51% total stake      |
| **Reward**           | Block reward + fees    | Transaction fees + tips  |
| **Finality**         | Probabilistic          | Probabilistic → Absolute |
| **Ví dụ**            | Bitcoin, Ethereum cũ   | Ethereum mới, Cardano    |
...
### 7.6. Hybrid Approach (Kết hợp cả hai)

Trong một số trường hợp, bạn có thể kết hợp cả hai nền tảng:

#### 🔗 Use Case: Supply Chain + Token Rewards
...
│                                             │
│  Org1 ←→ Org2 ←→ Org3                      │
│  (Product tracking, prices, contracts)      │
│                                             │
│         ↓ (Anchor/Proof)                   │
│                                             │
│         Ethereum                            │
│    (Public token & rewards)                 │
│                                             │
│  - Issue reward tokens                      │
...
```javascript
// Step 1: Fabric - Track product privately
async function trackProduct(ctx, productId, location) {
  const product = {
    productId,
    location,
    timestamp: Date.now(),
    handler: ctx.clientIdentity.getID(),
  };
  await ctx.stub.putState(productId, Buffer.from(JSON.stringify(product)));
...
// Step 2: Ethereum - Anchor proof publicly
async function anchorProof(proofHash, productId) {
  const contract = new ethers.Contract(contractAddress, abi, signer);

  // Store proof on Ethereum (public)
  const tx = await contract.anchorProof(productId, proofHash);
  await tx.wait();

  console.log("Proof anchored on Ethereum:", tx.hash);
}
...
async function verifyProduct(productId) {
  ...
  const onChainHash = await contract.getProof(productId);

  // Compare with actual product data
  // (Product details stay private in Fabric)
...
- ✅ Privacy cho business data (Fabric)
- ✅ Public verification (Ethereum)
- ✅ Token incentives (Ethereum)
- ✅ High performance (Fabric)

---

### 7.7. Cost Comparison (So sánh chi phí)
...
- Audit: $5,000 - $50,000
- Deployment: $500 - $5,000 (gas fees)

Total: $15,500 - $105,000
```

**Ongoing Costs:**

```
- Gas fees per transaction: $1 - $50
- Monthly (1000 tx): $1,000 - $50,000
- Yearly: $12,000 - $600,000

⚠️ Chi phí biến động theo gas price
⚠️ Có thể giảm bằng Layer 2
```
...
// Transfer ERC20: ~65,000 gas
// At 50 gwei: 65,000 * 50 = 3,250,000 gwei = 0.00325 ETH
// At $3,000/ETH: $9.75 per transfer

// Uniswap swap: ~150,000 gas
// At 50 gwei: 150,000 * 50 = 7,500,000 gwei = 0.0075 ETH
// At $3,000/ETH: $22.5 per swap
```

---

#### 💰 Hyperledger Fabric Costs

**Setup Costs:**

```
- Chaincode Development: $10,000 - $50,000
...
- Infrastructure (AWS/Azure):
  - 3 Peer nodes: $500/month
  - 3 Orderer nodes: $500/month
  - CA: $100/month
  - Total: $1,100/month = $13,200/year

- Maintenance: $2,000 - $5,000/month
- Total yearly: $37,200 - $73,200

✅ Chi phí cố định, dễ dự đoán
✅ Không phụ thuộc số lượng transactions
```

**Infrastructure Example:**

```yaml
# AWS Infrastructure for Fabric
Peer Nodes (3x):
  - EC2 t3.large: $0.0832/hour x 3 = $180/month
  - EBS 100GB: $10/month x 3 = $30/month

Orderer Nodes (3x):
  - EC2 t3.medium: $0.0416/hour x 3 = $90/month
  - EBS 50GB: $5/month x 3 = $15/month
...
Total: ~$412/month (minimal setup)
```

---

### 7.8. Migration Path (Lộ trình chuyển đổi)

#### 🚀 Từ Centralized Database → Blockchain

**Scenario 1: Start with Ethereum**

```
Phase 1: POC (2-3 months)
  - Deploy testnet
...
**Scenario 2: Start with Fabric**

```
Phase 1: POC (3-4 months)
  - Network setup
  - Basic chaincode
  - 2-3 organizations
  Cost: $20,000 - $50,000

Phase 2: Pilot (4-6 months)
  - Production network
  - Full consortium
...
Chúc mừng bạn đã hoàn thành tài liệu đào tạo về **Phát triển Blockchain (Ethereum/EVM)**! 🎉

### 📚 Những gì bạn đã học:

**Phần 1:** Chuyển tiền và Transaction

- ETH vs ERC20
- transfer/approve/transferFrom
- Nonce, Gas, Confirmations

**Phần 2:** Ví, Ký và Xác thực
...
- Follow Ethereum roadmap
- Join developer communities
- Attend conferences/hackathons
...
- Twitter: Follow core developers
...
- 📧 Email: dev@kaopiz.com
- 💬 Slack: #blockchain-dev
- 📚 Internal Wiki: wiki.kaopiz.com/blockchain

---

### 🙏 Lời cảm ơn

Cảm ơn bạn đã dành thời gian học tập và phát triển kỹ năng blockchain. Chúc bạn thành công trên con đường trở thành Blockchain Developer! 🚀
...
_Phiên bản: 1.0 | Cập nhật lần cuối: 16/12/2025_
